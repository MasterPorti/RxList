...
export async function POST(req:Request){
  ...
  const floor=b.floor?Number(b.floor):undefined;const bed=b.bed?Number(b.bed):undefined;
  if(floor===undefined||bed===undefined)return NextResponse.json({error:"patient_bed_required"},{status:400});
  if(![1,2,3,4].includes(floor)||!floorHasRoom(c.store,floor as 1|2|3|4,bed))return NextResponse.json({error:"bed_unavailable"},{status:409});
