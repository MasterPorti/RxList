...
export default function DoctorOperationsDashboard() {
  ...
  return createPortal(<div className="operations-enhanced">
    <div className="enhanced-header"><div><span className="eyebrow">Panel clínico</span><h1>Seguimiento de pacientes</h1><p>Pisos, personal, tareas y evolución de signos vitales en una sola vista.</p></div><div className="enhanced-header-actions"><div className="enhanced-counter"><strong>{pending.length}</strong><span>tareas por realizar</span></div><Link className="enhanced-chat-link" href="/chat">Ir al chat <span>↗</span></Link></div></div>
    ...
    <nav className="enhanced-mobile-nav" aria-label="Accesos rápidos del dashboard"><button onClick={() => window.scrollTo({ top: 0, behavior: "smooth" })}>Resumen</button><button onClick={() => document.querySelector(".enhanced-floors")?.scrollIntoView({ behavior: "smooth" })}>Pisos</button><Link href="/chat">Chat</Link><button onClick={() => document.querySelector(".enhanced-patients")?.scrollIntoView({ behavior: "smooth" })}>Pacientes</button><button onClick={() => document.querySelector(".enhanced-task-columns")?.scrollIntoView({ behavior: "smooth" })}>Tareas</button></nav>
  </div>, target);
}
