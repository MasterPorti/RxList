...
export default function PatientDraftForm() {
  ...
  useEffect(() => {
    ...
    return () => { window.removeEventListener("rxlist:patient-draft", open); window.removeEventListener("rxlist:chat-closed", close); };
  }, []);
  ...
  function submit(event: React.FormEvent) {
    ...
    if (!draft) return;
    const message = `Registra al paciente ${draft.fullName}, fecha de nacimiento ${draft.birthDate}, motivo de ingreso ${draft.reason}, alergias ${draft.allergies}, contacto de emergencia ${draft.emergencyContact} teléfono ${draft.emergencyPhone}, piso ${draft.floor}. La cama debe asignarse automáticamente.`;
    window.dispatchEvent(new CustomEvent("rxlist:patient-draft-submit", { detail: message }));
    setDraft(null);
  }
