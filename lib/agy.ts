...
export async function proposeWithAgy(message: string, doctor: Doctor, context?: Pick<Store,"floors"|"patients"|"shifts"|"medications"|"tasks">) {
  message = "REGLA OPERATIVA VIGENTE: los turnos day son 05:00-17:00 y los turnos night son 17:00-05:00. Usa siempre esos horarios. REGLA DE INGRESO: registrar un paciente significa que ya llegó al hospital; el mínimo obligatorio es datos del paciente, causa de ingreso, alergias, contacto de emergencia, piso y cama. El contacto de emergencia requiere nombre y teléfono. Para dar de alta a un paciente requiere la causa de salida.\n" + message;
  ...
  try {
    ...
    const intentAction = ["create_nurse","update_floor","create_patient","assign_patient","move_patient","discharge_patient","create_shift","create_medication","create_task"].includes(String(data.intent)) ? String(data.intent) : undefined;
    ...
    const incomplete = (data.operations as any[] | undefined)?.some(op =>
      (op.action === "create_patient" && (!op.fullName || !op.birthDate || !op.reason || !op.allergies || !op.emergencyContact || !op.emergencyPhone || typeof op.floor !== "number" || typeof op.bed !== "number")) ||
    ...
    if (incomplete) {
      ...
      const retryIntent = ["create_nurse","update_floor","create_patient","assign_patient","move_patient","discharge_patient","create_shift","create_medication","create_task"].includes(String(data.intent)) ? String(data.intent) : undefined;
