...
const BED_RULE = "REGLA DE CAMAS: para un paciente nuevo, la cama puede omitirse y el servidor intentará asignar una cama libre. Para assign_patient o move_patient, la cama también es opcional: si el doctor solo indica el piso, traslada al paciente al piso destino y deja la cama sin asignar; no digas que se asignará automáticamente. Solo valida o menciona una cama cuando el doctor la indique explícitamente.";

export function buildAgyPrompt(message: string, doctor: Doctor, context?: Pick<Store,"floors"|"patients"|"shifts"|"medications"|"tasks">) {
  const operational = "REGLA OPERATIVA VIGENTE: los turnos day son 05:00-17:00 y los turnos night son 17:00-05:00. Usa siempre esos horarios. REGLA DE INGRESO: registrar un paciente significa que ya llegó al hospital; el mínimo obligatorio es datos del paciente, causa de ingreso, alergias, contacto de emergencia y piso. La cama es opcional porque el servidor asignará automáticamente una cama libre. El contacto de emergencia requiere nombre y teléfono. Para dar de alta a un paciente requiere la causa de salida.\n";
