...
export function buildAgyPrompt(message: string, doctor: Doctor, context?: Pick<Store,"floors"|"patients"|"shifts"|"medications"|"tasks"|"vitals">) {
  ...
  const reportClarityRule = "REGLA DE INTENCIÓN DE REPORTE: sólo genera un informe listo para imprimir y sólo uses lenguaje de informe si el mensaje actual del doctor contiene explícitamente reporte, informe o una variante ortográfica evidente. Un mensaje como dame información, dame la información clínica o qué información tienes es una consulta normal y no debe heredar la intención de reporte del historial anterior.";
