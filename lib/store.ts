...
const seed:Store={schemaVersion:3,revision:1,users:[{id:"admin",name:"Administración",email:"admin@rxlist.local",passwordHash:hashPassword("RXList-Admin-2026!","rxlist-admin-salt"),role:"admin"}],floors,patients:[],shifts:[],medications:[],tasks:[],vitals:[],audit:[],chatHistory:{},settings:{agyEnabled:true}};
...
function migrate(raw:Partial<Store>):Store{const s={...seed,...raw};s.schemaVersion=3;s.floors=s.floors?.length?s.floors:floors;s.patients=s.patients||[];s.shifts=s.shifts||[];s.medications=s.medications||[];s.tasks=s.tasks||[];s.vitals=s.vitals||[];s.audit=s.audit||[];s.chatHistory=s.chatHistory||{};s.settings={agyEnabled:raw.settings?.agyEnabled ?? true};addDemoClinicalData(s as Store);return s as Store}
