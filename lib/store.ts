...
import { demoStore, expandDemoStore } from "./demo-data";
...
const file=path.join(process.cwd(),"data","rxlist.json"); let client:ReturnType<typeof createClient>|null=null; let templateLoaded=false;
...
async function local(){if(!templateLoaded){await fs.mkdir(path.dirname(file),{recursive:true});const s=demoStore();await fs.writeFile(file,JSON.stringify(s,null,2));templateLoaded=true;return s}try{const raw=JSON.parse(await fs.readFile(file,"utf8"));const s=migrate(raw);if(completeNurseEmails(s)||s.patients.length>(raw.patients?.length||0)||s.vitals.length>(raw.vitals?.length||0)||s.tasks.length>(raw.tasks?.length||0))await fs.writeFile(file,JSON.stringify(s,null,2));return s}catch{const s=demoStore();await fs.writeFile(file,JSON.stringify(s,null,2));return s}}
export async function getStore(){if(process.env.REDIS_URL){client ??=createClient({url:process.env.REDIS_URL});if(!client.isOpen)await client.connect();if(!templateLoaded){const s=demoStore();completeNurseEmails(s);await client.set("rxlist:store",JSON.stringify(s));templateLoaded=true;return s}const raw=await client.get("rxlist:store");if(raw){const parsed=JSON.parse(raw);const s=migrate(parsed);if(completeNurseEmails(s)||s.patients.length>(parsed.patients?.length||0)||s.vitals.length>(parsed.vitals?.length||0)||s.tasks.length>(parsed.tasks?.length||0))await client.set("rxlist:store",JSON.stringify(s));return s}const s=demoStore();completeNurseEmails(s);await client.set("rxlist:store",JSON.stringify(s));return s}return local()}
